-- Auto Save Item Script - Fixed version with item selection and character respawn handling
...
local function getHumanoidOrWait()
...
local function equipItem(item)
    if item and item.Parent == Backpack then
        -- Đợi nhân vật hồi sinh nếu bị chiết
        local humanoid = nil
        while not humanoid do
            local char = getCharacterOrWait()
            humanoid = char:FindFirstChild("Humanoid")
            if not humanoid then task.wait(0.1) end
        end
